...
class GroupCoachingDayAfterMeetingReminder extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'group-coaching:day-after';
     ...
     * @var string
     */
    protected $description = 'Sends out a meeting reminder to a member a day after a Lesson.';
    ...
    {
        // Get the lesson settings for one day after reminder
        $lesson_meeting_settings = GroupCoachingLessonMeetingSetting::where('one_day_after_reminder', 1)->whereNull('deleted_at')->with(['lessonmeeting' => function ($query) {
        ...
        foreach ($lesson_meeting_settings as $key => $each) {
            ...
            // Check time threashhold i.e one day after
            $check = $this->checkIfDayAfter($lesson_meeting, $each);
    ...
    public function checkIfDayAfter($meeting, $msettings)
    {
        if ($meeting) {
            ...
            if ($meeting_date->isYesterday() && $today->greaterThan($meeting_date)) {
                $msettings->one_day_after_reminder = 0;
                $msettings->save();
                return true;
            }
        }
    ...
    public function getMailTemplate($lesson_id)
    {
        ...
        if($notification_template){
            ...
            $subject = $notification_template->one_day_after_sub;
            $template = $notification_template->one_day_after;
