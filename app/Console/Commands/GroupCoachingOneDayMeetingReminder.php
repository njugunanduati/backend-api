...
class GroupCoachingOneDayMeetingReminder extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'group-coaching:day-before';
     ...
     * @var string
     */
    protected $description = 'Sends out a meeting reminder to a member one day before a Lesson.';
    ...
    {
        // Get the lesson settings for one day before reminder
        $lesson_meeting_settings = GroupCoachingLessonMeetingSetting::where('one_day_reminder', 1)->whereNull('deleted_at')->with(['lessonmeeting' => function ($query) {
        ...
        foreach ($lesson_meeting_settings as $key => $each) {
            ...
            // Check time threashhold i.e one day before
            $check = $this->checkIfDayBefore($lesson_meeting, $each);
    ...
    public function checkIfDayBefore($meeting, $msettings)
    {
        if ($meeting) {
            ...
            if ($meeting_date->isTomorrow() && $today->lessThan($meeting_date)) {
                $msettings->one_day_reminder = 0;
                $msettings->save();
                return true;
            }
        }
    ...
    public function getMailTemplate($lesson_id)
    {
        ...
        if($notification_template){
            ...
            $subject = $notification_template->one_day_before_sub;
            $template = $notification_template->one_day_before;
