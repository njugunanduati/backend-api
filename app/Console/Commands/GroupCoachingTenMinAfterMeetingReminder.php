...
class GroupCoachingTenMinAfterMeetingReminder extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'group-coaching:ten-min-after';
     ...
     * @var string
     */
    protected $description = 'Sends out a meeting reminder to a member ten min after a Lesson.';
    ...
    {
        // Get the lesson settings for ten minutes after reminder
        $lesson_meeting_settings = GroupCoachingLessonMeetingSetting::where('ten_min_after_reminder', 1)->whereNull('deleted_at')->with(['lessonmeeting' => function ($query) {
        ...
        foreach ($lesson_meeting_settings as $key => $each) {
            ...
            // Check time threashhold i.e ten min after
            $check = $this->checkIfTenMinAfter($lesson_meeting, $each);
    ...
    public function checkIfTenMinAfter($meeting, $msettings)
    {
        ...
        if ($meeting) {
            ...
            if ($today->diffInMinutes($meeting_day) === 10 && $today->greaterThanOrEqualTo($meeting_day)) {
                $msettings->ten_min_after_reminder = 0;
                $msettings->save();
                return true;
            }
        }
    ...
    public function getMailTemplate($lesson_id)
    {
        ...
        if($notification_template){
            ...
            $subject = $notification_template->ten_min_after_sub;
            $template = $notification_template->ten_min_after;
