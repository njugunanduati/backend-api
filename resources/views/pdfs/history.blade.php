  ...
  <body>
    ...
    <section class="notecontainer">
        @foreach ($notes as $note)
            ...
            @if (count($note->reminder) > 0)
            <section class="box">
                <section class="heading">Commitments</section>
                    <section class="content">
                        @foreach ($note->reminder as $item)
                            <section>
                                <div><span>Type: </span> {{($item->type == 'client')? 'Client Commitment' : 'Coach Commitment'}}</div>
                                <div><span>Status: </span> {{($item->status == 0)? 'Not Done' : ($item->status == 1)? 'In Progress' : 'Complete'}}</div>
                                <div><span>Due: </span> {{Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $item->reminder_date)->toDayDateTimeString()}}</div>
